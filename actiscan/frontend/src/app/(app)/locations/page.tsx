...
export function LocationsPage() {
  ...
  const { register, handleSubmit, reset } = useForm<{ name: string; floor?: string; building?: string }>()
  const mutation = useMutation({
    mutationFn: (d: any) => api.post("/api/locations", d),
    ...
    <div>
      ...
      <Modal open={show} onClose={() => { setShow(false); reset() }} title="Nueva ubicación">
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-3">
          <Input label="Nombre *" placeholder="Ej. Almacén Norte" {...register("name", { required: true })} />
          <div className="grid grid-cols-2 gap-3">
            <Input label="Piso" placeholder="Piso 3" {...register("floor")} />
            <Input label="Edificio" placeholder="Torre A" {...register("building")} />
          </div>
          <div className="flex gap-2 pt-1">
            <Button type="button" variant="secondary" className="flex-1 justify-center" onClick={() => setShow(false)}>Cancelar</Button>
...
export function CategoriesPage() {
  ...
  const mutation = useMutation({
    mutationFn: (d: any) => api.post("/api/categories", d),
  ...
  return (
    <div>
      ...
      <Modal open={show} onClose={() => { setShow(false); reset() }} title="Nueva categoría">
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-3">
          <Input label="Nombre *" placeholder="Ej. Equipos de Cómputo" {...register("name", { required: true })} />
